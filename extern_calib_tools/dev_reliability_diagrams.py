# evaluation metrics for ECE and SCE
# Modified from https://github.com/hollance/reliability-diagrams
import os
import numpy as np
import matplotlib.pyplot as plt

def get_sce_calibrationv2(probs, pred_labels, true_labels, nclass, num_bins=10):
    """
    Based on https://github.com/Jonathan-Pearce
    and https://arxiv.org/pdf/1904.01685.pdf
    ...
    for cls in range(nclass):
        #conf_by_class[cls] = confidences[pred_labels == cls]
        conf_by_class[cls] = probs[cls, :]
        ind_by_class[cls] = np.digitize(conf_by_class[cls], bins, right=True)

    bin_accuracies = np.zeros([num_bins, nclass], dtype=np.float)
    bin_confidences = np.zeros([num_bins, nclass], dtype=np.float)
    bin_counts = np.zeros([num_bins, nclass], dtype=np.int)

    for b in range(num_bins):
        for cls in range(nclass):
            selected = np.where(ind_by_class[cls] == b + 1)[0] # for the current class and the current bin
            if len(selected) > 0:
                #bin_accuracies[b, cls] = np.mean(true_by_class_pred[cls][selected] == pred_by_class_pred[cls][selected])
                cls_bin_tp = (true_labels[selected] == cls) # implementation by the author https://github.com/JeremyNixon/uncertainty-metrics-1

                bin_accuracies[b, cls] = np.mean(cls_bin_tp)
                bin_confidences[b, cls] = np.mean(conf_by_class[cls][selected])
                bin_counts[b, cls] = len(selected)

    gaps = np.abs(bin_accuracies - bin_confidences) # nbin * nc
    weighted_gaps = gaps * bin_counts
    sce = weighted_gaps.sum() / np.prod(true_labels.shape) / nclass

    return {"static_calibration_errorv2": sce }
