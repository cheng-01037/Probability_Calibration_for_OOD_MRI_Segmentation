...
class Efficient_AllScore(nn.Module):
    """
        Adding precision and recall for the ease of analysis
        WARNING: This is not a loss function! don't use this. To train a network, use ``SoftDiceLoss'' above.
        NOTE: modified by co1818 to enforce binary values for logits, which enables good visualization
        NOTE: double-modified by co1818 after a careful look
        NOTE: thir by co1818 for memory efficiency
    """
    ...
    def __init__(self, n_classes, ignore_chan0 = True):
        ...
        Args:
            ignore_chan0: ignore the channel 0 of the segmentation (usually for the background)
        """
        super(Efficient_AllScore, self).__init__()
        self.one_hot_encoder = One_Hot(n_classes).forward
        self.n_classes = n_classes
        self.ignore = ignore_chan0
    ...
    def forward(self, global_input, global_target, dense_input = False, cutfold = 5):
        ...
        global_inter = 0 # true positive
        global_nz_pred = 0
        global_nz_gth = 0
        global_pred_pos = 0 # false positive
        global_real_pos = 0 # false negative
        ...
        for ii in range( niter ):
            ...
            try:
                ...
                # some wierd bug which might have something to do with configuration of cuda memory
                MAGIC_NUMBER = 14000000
                if input.shape[0] < MAGIC_NUMBER:
                    raise ValueError
        ...
        global_p_sum = global_nz_pred + global_nz_gth + smooth # sum of positives

        dice_score      = 2.0 * global_inter / global_p_sum
        prec_score      = global_inter / (global_nz_pred + smooth)
        recall_score    = global_inter / (global_nz_gth + smooth)

        return {'dice': dice_score.data.cpu().numpy(), 'prec': prec_score.data.cpu().numpy(), 'recall': recall_score.data.cpu().numpy()}
